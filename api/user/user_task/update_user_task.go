...
func UpdateUserTaskApi(c *gin.Context) {
	...
	userQuestion, err := updateUserTask(c, req.ID, req.UserID, req.TaskID, req.Count, req.TargetID, req.Addtion, req.IsFinished)
	if err != nil {
...
func updateUserTask(c *gin.Context, id, userId, taskId string, count int64, targetId, additon string, isFinished int64) (*model.UserTask, error) {
	...
	if isFinished != 0 {
		userTask.IsFinished = isFinished
	}
